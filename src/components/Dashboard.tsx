...
export default function Dashboard() {
  ...
  // Memoized data for delete modals
  const deletingChainTransactions = useMemo(() => 
    deletingChainId ? transactions.filter(t => t.chainId === deletingChainId) : [],
    [deletingChainId, transactions]
  );
  
  const deletingChainInfo = useMemo(() => 
    deletingChainId ? chains.find(c => c.id === deletingChainId) : null,
    [deletingChainId, chains]
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      {!loading && (
        <main className={`py-8 ${isMobile ? 'px-2' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'}`}>
          <h1 className="sr-only">Options Trading Dashboard - {selectedPortfolioId ? portfolios.find(p => p.id === selectedPortfolioId)?.name : 'All Portfolios'}</h1>
          ...
          {activeTab === 'risk' ? (
            ...
            <SummaryView transactions={portfolioFilteredTransactions} />
          )}
        </main>
      )}
      ...
      <DeleteChainModal
        ...
        chainTransactions={deletingChainTransactions}
        chainInfo={deletingChainInfo}
      />
    </div>
  );
}
