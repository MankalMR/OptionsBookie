...
export const calculateSmartCapital = (
  ...
  for (const [stock, stockTransactions] of byStock.entries()) {
    ...
    if (stockCapital > 0) {
      //   debugLog(`[DEBUG] ${stock}: capital=${stockCapital}, trades=${stockTradeCount}, sequential=${stockIsSequential && transactionsForDetection.length > 1}, hasOverlap=${stockOverlapResult?.hasOverlap}`);
      breakdown.push({
        stock,
  ...
  // debugLog(`[DEBUG] Final totalCapital: ${totalCapital}`);
