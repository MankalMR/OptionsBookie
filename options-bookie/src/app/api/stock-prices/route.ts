...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.debug('ALPHA_VANTAGE_KEY available:', !!process.env.ALPHA_VANTAGE_KEY);
    logger.debug('FINNHUB_API_KEY available:', !!process.env.FINNHUB_API_KEY);
    logger.debug('Available providers:', StockPriceFactory.getAvailableProviders());
    ...
    if (symbolList.length === 1) {
      ...
      if (!result) {
        logger.debug('Cached service failed, trying pure Finnhub as last resort');
      ...
      if (!hasAnyResults) {
        logger.debug('Cached service failed for all symbols, trying pure Finnhub as last resort');
        stockService = StockPriceFactory.initialize('finnhub');
        result = await stockService.getMultipleStockPrices(symbolList);
