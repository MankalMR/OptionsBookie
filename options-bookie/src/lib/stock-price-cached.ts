...
export class CachedStockService {
  ...
  async getStockPrice(symbol: string): Promise<StockPriceResponse | null> {
    try {
      ...
      logger.debug(`Checking cache for ${symbol}`);
      const cached = await this.getCachedPrice(symbol);
      if (cached) {
        logger.debug(`✅ Using cached price for ${symbol}: $${cached.price}`);
        return cached;
      }
      logger.debug(`❌ No cached price found for ${symbol}`);

      // If not cached and Alpha Vantage is available, try to fetch from Alpha Vantage
      if (process.env.ALPHA_VANTAGE_KEY) {
        logger.debug(`Fetching fresh price for ${symbol} from Alpha Vantage`);
        try {
          const priceData = await alphaVantageStockService.getStockPrice(symbol);

          if (priceData) {
            // Cache the result in shared cache
            logger.debug(`💾 Caching price for ${symbol}: $${priceData.price} for 1 day`);
            await this.cachePrice(symbol, priceData);
            return priceData;
          }
        } catch (error) {
          logger.debug(`Alpha Vantage failed for ${symbol}, returning null:`, error);
        }
      } else {
        logger.debug(`ALPHA_VANTAGE_KEY not available, returning null for ${symbol}`);
      }
  ...
  async getMultipleStockPrices(symbols: string[]): Promise<Record<string, StockPriceResponse | null>> {
    ...
    logger.debug(`📦 Checking cache for ${symbols.length} symbols: ${symbols.join(', ')}`);

    symbols.forEach(symbol => {
      if (cachedResults[symbol]) {
        logger.debug(`✅ Using cached price for ${symbol}: $${cachedResults[symbol]!.price}`);
        results[symbol] = cachedResults[symbol];
      } else {
        logger.debug(`❌ No cached price for ${symbol}`);
        symbolsToFetch.push(symbol);
      }
    });

    logger.debug(`🔄 Need to fetch ${symbolsToFetch.length} symbols from Alpha Vantage: ${symbolsToFetch.join(', ')}`);

    // Fetch uncached symbols from Alpha Vantage if available
    if (symbolsToFetch.length > 0) {
      if (process.env.ALPHA_VANTAGE_KEY) {
        logger.debug(`Fetching fresh prices for: ${symbolsToFetch.join(', ')} from Alpha Vantage`);
        try {
          ...
          logger.debug(`Alpha Vantage failed for symbols: ${symbolsToFetch.join(', ')}, returning null:`, error);
          symbolsToFetch.forEach(symbol => {
            results[symbol] = null;
          });
        }
      } else {
        logger.debug(`ALPHA_VANTAGE_KEY not available, returning null for uncached symbols: ${symbolsToFetch.join(', ')}`);
        symbolsToFetch.forEach(symbol => {
          results[symbol] = null;
        });
      }
