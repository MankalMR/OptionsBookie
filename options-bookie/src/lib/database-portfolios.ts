...
export const portfolioDb = {
  ...
  async setDefaultPortfolio(id: string, userId: string): Promise<Portfolio | null> {
    try {
      debugLog('setDefaultPortfolio - ID:', id, 'User ID:', userId);
      ...
      if (!existingPortfolio) {
        debugLog('Portfolio not found for user');
        return null;
      }

      debugLog('Portfolio found:', existingPortfolio);
      ...
      debugLog('Successfully set default portfolio:', data);
      return rowToPortfolio(data);
    } catch (error) {
