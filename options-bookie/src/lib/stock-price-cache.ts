  ...
  async cachePrice(symbol: string, priceData: StockPriceResponse): Promise<void> {
    try {
      ...
      if (error) {
        ...
        debugLog(`Cached price for ${symbol} until ${expiresAt.toISOString()}`);
      }
    } catch (error) {
      console.error('Error caching price:', error);
    }
    ...
    if (cacheData.length > 0) {
      try {
        ...
        if (error) {
          ...
          debugLog(`Cached ${cacheData.length} prices until ${expiresAt.toISOString()}`);
        }
      } catch (error) {
        console.error('Error caching multiple prices:', error);
      }
  ...
  async clearExpiredCache(): Promise<void> {
    try {
      ...
      if (error) {
        ...
        debugLog('Cleared expired cache entries');
      }
    } catch (error) {
      console.error('Error clearing expired cache:', error);
    }
